...
export function getSwwwCommandFromConfiguration(
  ...
  const command = `swww img "${imagePath}" ${monitor.length > 0 ? `--outputs ${monitor}` : ''
    } --resize="${swwwConfig.resizeType}" --fill-color "${swwwConfig.fillColor
    }" --filter ${swwwConfig.filterType} --transition-type ${transitionType} --transition-step ${swwwConfig.transitionStep} --transition-duration ${swwwConfig.transitionDuration
    } --transition-fps ${swwwConfig.transitionFPS} --transition-angle ${swwwConfig.transitionAngle
    } --transition-pos ${transitionPos} ${inverty} --transition-bezier ${swwwConfig.transitionBezier
...
async function runExternalScripts(imageFilePath: string) {
  for (let idx = 0; idx < configuration.scripts.length; idx++) {
    ...
    try {
      ...
      logger.error(error)
      notify("There was an error running your script, run with --logs flag to debug")
    }
  }
}
