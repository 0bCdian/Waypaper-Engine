...
| **Backend** (wallpaper backend) | An **external** tool the daemon invokes to apply a wallpaper: `awww`, `hyprpaper`, `feh`, `mpvpaper`, or `wayland-utauri`. The daemon does not embed them; they must be installed and on `PATH` (or reported unavailable — see `GET /backends`). |
...
| **wayland-utauri**              | Optional **wayland-utauri** binary for image, video, and **HTML** wallpapers on Wayland. It is a **separate** project; the daemon spawns/controls it through the `wayland-utauri` backend package, not the webview inside Electron.              |
