...
import { normalizeLegacyWaylandUtauriMonitorName } from "./monitorNames";

/**
 * Maps a live compositor monitor name to the image id shown on that display,
 * using GET /wallpaper/current slots. Handles legacy stable_id ↔ "Monitor N"
 * normalization and shared-wallpaper modes when per-monitor rows are missing
 * after compositor renames.
 */
export function resolveWallpaperImageId(
  ...
  const normName = normalizeLegacyWaylandUtauriMonitorName(monitorName);
  const legacy = slots.find(
    (s) => normalizeLegacyWaylandUtauriMonitorName(s.monitor_name) === normName,
  );
  if (legacy) return legacy.image_id;
