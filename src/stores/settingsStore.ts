...
import type { UnifiedConfig, ConfigSection, ConfigChangeEvent } from '../../shared/types/unifiedConfig';
...
interface SettingsStoreActions {
  ...
  saveConfig: (section: ConfigSection, key: string, value: unknown) => Promise<void>;
  ...
  // Validation
  validateConfig: (config: Partial<UnifiedConfig>) => Array<{ section: ConfigSection; key: string; message: string }>;
  clearErrors: () => void;
}
...
export const useSettingsStore = create<SettingsStore>()(
  ...
  errors: [],
      ...
      saveConfig: async (section: ConfigSection, key: string, value: unknown) => {
        ...
        try {
          if (window.API_RENDERER?.goDaemon?.setConfig) {
            ...
            console.log('🟢 SettingsStore: Config updated successfully', { section, key, value });
          } else {
          ...
          for (const [section, sectionConfig] of Object.entries(defaultConfig)) {
            for (const [key, value] of Object.entries(sectionConfig)) {
              if (section === 'backend' && key === 'swww') {
                ...
                for (const [swwwKey, swwwValue] of Object.entries(value)) {
                  await get().saveConfig('backend', `swww.${swwwKey}`, swwwValue);
                }
              } else {
      ...
      validateConfig: (config: Partial<UnifiedConfig>) => {
        const errors: Array<{ section: ConfigSection; key: string; message: string }> = [];
        
        // Add validation logic here based on your requirements
        // For now, just basic validation
        
        if (config.app?.images_per_page && (config.app.images_per_page < 1 || config.app.images_per_page > 100)) {
          errors.push({
            section: 'app',
            key: 'images_per_page',
        ...
        if (config.app?.image_history_limit && (config.app.image_history_limit < 1 || config.app.image_history_limit > 1000)) {
          errors.push({
            section: 'app',
