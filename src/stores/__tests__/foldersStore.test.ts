...
describe("useFoldersStore", () => {
  ...
  it("fetchFolders calls getFolders and sets state", async () => {
    ...
    mockAPI.goDaemon.getFolders = vi.fn().mockResolvedValue({ data: folders });
  ...
  it("navigateToFolder sets currentFolderId and fetches", async () => {
    mockAPI.goDaemon.getFolders = vi.fn().mockResolvedValue({ data: [] });
    mockAPI.goDaemon.getFolderPath = vi.fn().mockResolvedValue({ data: [] });
  ...
  it("createFolder calls API and appends to state", async () => {
    ...
    await act(async () => {
      const result = await useFoldersStore.getState().createFolder("New Folder", null);
      expect(result.name).toBe("New Folder");
    });

    expect(mockAPI.goDaemon.createFolder).toHaveBeenCalledWith("New Folder", null);
    const state = useFoldersStore.getState();
  ...
  it("renameFolder updates folder in state", async () => {
    ...
    mockAPI.goDaemon.getFolders = vi.fn().mockResolvedValue({ data: [original] });
    mockAPI.goDaemon.getImages = vi.fn().mockResolvedValue({
      data: [],
      pagination: { page: 1, per_page: 4, total_items: 0, total_pages: 0 },
    });
  ...
  it("deleteFolder removes from state", async () => {
    ...
    mockAPI.goDaemon.getFolders = vi.fn().mockResolvedValue({ data: [folder] });
    mockAPI.goDaemon.getImages = vi.fn().mockResolvedValue({
      data: [],
      pagination: { page: 1, per_page: 4, total_items: 0, total_pages: 0 },
    });
    ...
    expect(mockAPI.goDaemon.deleteFolder).toHaveBeenCalledWith(1, "keep_contents");
  });

  it("searchFolders returns results", async () => {
    const results = [sampleFolder(3, "Nature")];
    mockAPI.goDaemon.getFolders = vi.fn().mockResolvedValue({ data: results });
