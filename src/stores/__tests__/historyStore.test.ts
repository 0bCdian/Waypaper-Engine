...
import { sampleHistoryEntry, sampleImage } from "../../test/mocks/fixtures";
...
describe("useHistoryStore", () => {
  ...
  it("fetchHistory calls getImageHistory and resolves images", async () => {
    ...
    mockAPI.goDaemon.getImage = vi
      .fn()
      .mockImplementation((id: number) => Promise.resolve(sampleImage(id)));
  ...
  it("fetchHistory handles error", async () => {
    mockAPI.goDaemon.getImageHistory = vi.fn().mockRejectedValue(new Error("fetch failed"));
  ...
  it("loadMore appends older entries", async () => {
    ...
    const olderEntries = [...initial, sampleHistoryEntry(5, { set_at: "2024-12-31T00:00:00Z" })];
    mockAPI.goDaemon.getImageHistory = vi.fn().mockResolvedValue(olderEntries);
  ...
  it("clearHistory calls API and resets", async () => {
    ...
    mockAPI.goDaemon.getImage = vi
      .fn()
      .mockImplementation((id: number) => Promise.resolve(sampleImage(id)));
    mockAPI.goDaemon.clearImageHistory = vi.fn().mockResolvedValue({ status: "cleared" });
