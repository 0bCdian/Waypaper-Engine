...
async function downloadAndImportWallpaper(
	...
	return new Promise<number>((resolve, reject) => {
		...
		dispose = window.API_RENDERER.goDaemon.on(
			"image_processed",
			(data: unknown) => {
	...
	(set, get) => ({
		...
		search: async (apiKey) => {
			...
			try {
				...
				if (scrollMode === "infinite") {
					...
					const newItems = raw.data.filter(
					...
					set({
						...
						infiniteHighestPage: Math.max(
							get().infiniteHighestPage,
							filters.page,
						),
					});
				} else {
		...
		downloadImportAndSet: async (wp, monitor, mode) => {
			...
			try {
				...
				await window.API_RENDERER.goDaemon.setWallpaper(
					imageId,
					monitor,
					mode,
				);
		...
		setScrollMode: (mode) =>
			set((s) => ({
				scrollMode: mode,
				infiniteResults:
					mode === "infinite" ? [...s.results] : [],
				infiniteHighestPage:
					mode === "infinite" ? s.filters.page : 0,
			})),
		...
		downloadSelected: async (_apiKey) => {
			...
			const toDownload = allWallpapers.filter((w) =>
				selectedWallpapers.has(w.id),
			);
			...
			const workers = Array.from(
				{ length: Math.min(BATCH_CONCURRENCY, toDownload.length) },
				() => worker(),
			);
