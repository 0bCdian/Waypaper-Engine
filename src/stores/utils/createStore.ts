      ...
      if (typeof value === 'function') {
        wrappedStore[key as keyof T] = ((...args: any[]) => {
          try {
            ...
            set({ error: error instanceof Error ? error.message : 'Unknown error' } as any);
            throw error;
          }
        }) as any;
      }
  ...
  return (set, get, api) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      set = (partial, replace) => {
        ...
        if (end - start > 10) { // Log if update takes more than 10ms
...
export function withValidation<T>(
  ...
  return (set, get, api) => {
    ...
    const originalSet = set;
    set = (partial, replace) => {
      const newState = typeof partial === 'function' ? (partial as any)(get()) : partial;
