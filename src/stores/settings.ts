    ...
    (set, get) => ({
      ...
      loadSettings: async () => {
        ...
        try {
          ...
          if (window.API_RENDERER?.goDaemon) {
            const config = await window.API_RENDERER.goDaemon.getFrontendConfig();
            const loadedSettings = config.settings || defaultSettings;
      ...
      validateSettings: (settings: AppSettings): AppSettings => {
        ...
        Object.keys(defaultSettings).forEach(key => {
          ...
          if (typeof value === typeof defaultSettings[settingKey]) {
            ...
            validated[settingKey] = defaultSettings[settingKey];
          }
        });
        ...
        if (validated.playlistInterval < 1 || validated.playlistInterval > 3600) {
...
export const useIsSettingsLoading = () => useSettingsStore(state => state.isLoading);
export const useIsSettingsDirty = () => useSettingsStore(state => state.isDirty);
