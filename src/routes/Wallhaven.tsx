...
function WallhavenPage() {
	...
	}, [selectAllVisible, clearSelection, selectedWallpaper, selectWallpaper, selectedWallpapers.size]);
	...
	const displayResults =
		scrollMode === "infinite" ? infiniteResults : results;

	const handleScrollModeToggle = () => {
		const newMode: WallhavenScrollMode =
			scrollMode === "paginated" ? "infinite" : "paginated";
	...
	return (
		<div className="flex flex-col h-full overflow-hidden">
			...
			<div className="shrink-0 px-3 py-2 border-b border-base-content/10 flex flex-wrap items-center gap-2">
				...
				<div className="tooltip tooltip-bottom shrink-0" data-tip={scrollMode === "paginated" ? "Switch to infinite scroll" : "Switch to paginated"}>
					<button
						type="button"
						className={cn(
			...
			<div className={cn(
				...
				<span className={cn("text-sm font-medium transition-opacity", hasSelection ? "opacity-100" : "opacity-0")}>
				...
				<button
					type="button"
					className={cn("btn btn-xs btn-ghost transition-opacity", hasSelection ? "opacity-100" : "opacity-0 pointer-events-none")}
			...
			{/* Error */}
			{error && (
				<div className="shrink-0 px-4 py-2 bg-error/10 text-error text-sm">
					{error}
				</div>
			)}
				...
				<div className="shrink-0 p-4 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 border-t border-base-content/10">
					<div className="flex flex-col gap-1 min-w-0">
						<span className="font-mono text-sm">
							...
							{(wp.file_size / 1024 / 1024).toFixed(1)} MB
							&middot;{" "}
