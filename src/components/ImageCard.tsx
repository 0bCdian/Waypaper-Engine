...
function ImageCard({ Image }: ImageCardProps) {
	...
	const memoizedImage = useMemo(
		...
		[
			Image.id,
			Image.name,
			Image.selection?.isSelected,
			Image.selection?.isChecked,
		],
	);

	const [selected, setSelected] = useState(
		memoizedImage.selection?.isSelected ?? false,
	);
	...
	const getThumbnailSrc = () => {
		...
		if (screenWidth >= 2560) {
			...
			thumbnailPath =
				memoizedImage.thumbnails["1080p"] || memoizedImage.thumbnails["720p"];
		} else {
