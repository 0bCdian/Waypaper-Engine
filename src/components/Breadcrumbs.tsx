...
function DroppableBreadcrumb({
  ...
  const dropData = useMemo<DropTargetData>(() => ({ type: "breadcrumb", folderId }), [folderId]);
...
function Breadcrumbs() {
  ...
  return (
    <nav className="flex items-center gap-1 px-4 py-2 text-sm" aria-label="Breadcrumb">
        ...
        breadcrumbPath.map((folder, index) => {
          ...
          return (
            <span key={folder.id} className="flex items-center gap-1">
              ...
              <DroppableBreadcrumb folderId={folder.id} droppableId={`breadcrumb-${folder.id}`}>
                {isLast ? (
