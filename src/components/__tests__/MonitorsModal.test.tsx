...
describe("MonitorsModal", () => {
	it("renders title and display mode dropdown", () => {
		...
		expect(
			screen.getByText("Wallpaper per display"),
		).toBeInTheDocument();
		expect(
			screen.getByText("Stretch single wallpaper"),
		).toBeInTheDocument();
		expect(
			screen.getByText("Clone single wallpaper"),
		).toBeInTheDocument();
	});

	it("renders monitor entries from store", () => {
		mockMonitorsList = [
			makeMonitor("HDMI-A-1", false),
			makeMonitor("DP-1", false, 1920, 0),
		];
	...
	it("shows error when no monitors selected and Save clicked", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("Select at least one display"),
			).toBeInTheDocument();
		});
	});
	...
	it("shows error about needing 2+ displays in clone mode", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("Select at least two displays"),
			).toBeInTheDocument();
		});
	});
