...
export const StartupIntro: React.FC<StartupIntroProps> = ({ onFinish }) => {
  ...
  useEffect(() => {
    ...
    // Fallback: call finishOnce after hold + exit animation duration in case
    // the CSS transition does not fire (e.g. in test environments without a real browser).
    const fallback = window.setTimeout(finishOnce, OVERLAY_HOLD_MS + EXIT_S * 1000 + 100);
    return () => {
