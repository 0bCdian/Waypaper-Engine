...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
	(
		...
		// Base classes
		const baseClasses =
			"btn relative inline-flex items-center justify-center font-medium transition-all duration-200 focus:outline-hidden focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed";
		...
		return (
			...
			>
				...
				{!loading && (
					<>
						{/* Start icon */}
						{startIcon && <span className="mr-2 shrink-0">{startIcon}</span>}
						...
						{/* End icon */}
						{endIcon && <span className="ml-2 shrink-0">{endIcon}</span>}
					</>
				)}
			</button>
		);
...
export const IconButton = forwardRef<HTMLButtonElement, IconButtonProps>(
	({ icon, "aria-label": ariaLabel, ...props }, ref) => (
		<Button ref={ref} circular aria-label={ariaLabel} {...props}>
			{icon}
		</Button>
	),
);
