...
export const BackendSettingsSection: React.FC<BackendSettingsSectionProps> = ({
  ...
  const { config, saveConfig, errors, showAdvancedSettings, setShowAdvancedSettings } = useSettingsStore();
  const section: ConfigSection = 'backend';
  const [expandedAccordions, setExpandedAccordions] = useState<Set<string>>(new Set(['general']));
  ...
  const swwwSettings: SettingField[] = [
    ...
    },
    {
      ...
      description: 'Bezier curve parameters for smooth transitions (x1,y1,x2,y2)',
      type: 'text',
  ...
  const getFieldError = (key: string) => {
    return errors.find(error => error.section === section && error.key === key);
  };
  ...
  const renderField = (field: SettingField) => {
    const currentValue = field.key.startsWith('swww.') 
      ? config?.backend?.swww?.[field.key.replace('swww.', '') as keyof typeof config.backend.swww]
  ...
  const renderAccordion = (id: string, title: string, fields: SettingField[], description?: string) => {
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <div className="border-b border-base-300 pb-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-base-content">Backend Settings</h2>
      ...
      <div className="space-y-4">
        ...
        {/* swww Settings */}
        {config?.backend?.type === 'swww' && renderAccordion(
          'swww',
        ...
        {config?.backend?.type === 'feh' && (
          <div className="alert alert-info">
            ...
            <span className="text-sm">feh backend selected. Additional feh-specific settings will be available in a future update.</span>
          </div>
        )}
        ...
        {config?.backend?.type === 'nitrogen' && (
          <div className="alert alert-info">
            ...
            <span className="text-sm">nitrogen backend selected. Additional nitrogen-specific settings will be available in a future update.</span>
          </div>
        )}
        ...
        {config?.backend?.type === 'custom' && (
          <div className="alert alert-warning">
            ...
            <span className="text-sm">Custom backend selected. You'll need to configure custom scripts manually.</span>
          </div>
        )}
      </div>
    </div>
