...
export const DaemonSettingsSection: React.FC<DaemonSettingsSectionProps> = ({
  ...
  const renderField = (field: SettingField) => {
    const currentValue = config?.daemon?.[field.key as keyof typeof config.daemon];
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <div className="border-b border-base-300 pb-4">
        <h2 className="text-lg font-semibold text-base-content">Daemon Settings</h2>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {settingsFields.filter(field => 
            ['database_path', 'images_dir', 'thumbnails_dir', 'monitors_state_file'].includes(field.key)
          ).map(renderField)}
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {settingsFields.filter(field => 
            ['log_level', 'log_file', 'log_max_size', 'log_max_age', 'log_max_backups'].includes(field.key)
          ).map(renderField)}
        </div>
      </div>
    </div>
