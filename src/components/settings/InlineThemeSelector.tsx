...
export const InlineThemeSelector: React.FC<InlineThemeSelectorProps> = ({
  ...
  const filteredThemes = categoryFilter
    ? availableThemes.filter(
        (theme) => theme.category === categoryFilter || theme.category === "mixed",
      )
    : availableThemes;
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <div className="flex items-center gap-3 bg-base-200 rounded-lg px-4 py-3">
        <span className="text-sm font-medium text-base-content/70">Active:</span>
        <span className="badge badge-primary capitalize">{currentTheme}</span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {filteredThemes.map((theme) => (
          <input
            ...
            className={cn("btn btn-sm", theme.name === currentTheme && "btn-primary")}
