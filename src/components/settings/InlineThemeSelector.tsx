...
const getThemePreviewColors = (themeName: string) => {
  ...
  const themeColors: Record<string, { primary: string; secondary: string; accent: string; background: string }> = {
    'light': {
      primary: '#3b82f6',
      secondary: '#64748b',
      accent: '#f59e0b',
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <div className="bg-base-200 rounded-lg p-4">
        <div className="flex items-center gap-4">
          <div className="text-sm font-medium text-base-content">Current Theme</div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-full h-16 rounded border border-base-300 overflow-hidden">
                      ...
                      >
                        {/* Preview elements */}
                        <div className="absolute top-2 left-2 w-8 h-4 rounded" style={{ backgroundColor: colors.primary }} />
                        <div className="absolute top-2 right-2 w-6 h-4 rounded" style={{ backgroundColor: colors.secondary }} />
                        <div className="absolute bottom-2 left-2 w-4 h-4 rounded" style={{ backgroundColor: colors.accent }} />
                        <div className="absolute bottom-2 right-2 w-6 h-2 rounded" style={{ backgroundColor: colors.primary }} />
                      </div>
                    </div>
                    ...
                    <div className="w-full space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-sm capitalize">{theme.displayName}</span>
                        {isActive && (
                          <svg
                    ...
                    {!theme.available && (
                      <div className="absolute inset-0 bg-base-100/80 rounded-lg flex items-center justify-center">
                        <span className="text-xs text-base-content/60">Unavailable</span>
                      </div>
                    )}
                  </button>
                );
      ...
      <div className="text-xs text-base-content/60 text-center pt-2 border-t border-base-300">
        {Object.values(themesByCategory).flat().length} theme{Object.values(themesByCategory).flat().length !== 1 ? 's' : ''} available
