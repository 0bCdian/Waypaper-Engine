...
const searchSuggestions: SearchSuggestion[] = [
  ...
  { section: 'app', key: 'sidebar_collapsed', label: 'Sidebar Collapsed', description: 'Start with sidebar collapsed', category: 'Layout' },
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    if (!showSuggestions || filteredSuggestions.length === 0) {
      ...
      if (event.key === 'Enter' && onNavigateToSection) {
        ...
        if (searchLower.includes('app') || searchLower.includes('application') || searchLower.includes('theme') || searchLower.includes('notification')) {
          targetSection = 'app';
        } else if (searchLower.includes('daemon') || searchLower.includes('log') || searchLower.includes('database')) {
          targetSection = 'daemon';
        } else if (searchLower.includes('backend') || searchLower.includes('swww') || searchLower.includes('transition')) {
        ...
        <div className="absolute z-50 w-full mt-1 bg-base-100 border border-base-300 rounded-lg shadow-lg p-4">
          <div className="text-sm text-base-content/60 space-y-2">
            ...
            <ul className="space-y-1 text-xs">
              ...
              <li>• Use <kbd className="kbd kbd-xs">↑</kbd> <kbd className="kbd kbd-xs">↓</kbd> to navigate suggestions</li>
              <li>• Press <kbd className="kbd kbd-xs">Enter</kbd> to select a suggestion</li>
              <li>• Press <kbd className="kbd kbd-xs">Esc</kbd> to clear search</li>
            </ul>
          </div>
        </div>
      )}
