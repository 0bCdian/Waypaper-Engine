...
async function initializeApp(): Promise<void> {
  try {
    console.log('Initializing Waypaper Engine...');
  ...
  app.on('before-quit', async () => {
    try {
      ...
      try {
        ...
        console.log('Go daemon was not connected, skipping stop');
      }
...
function setupDevTools(): void {
  if (process.env.NODE_ENV === 'development') {
    ...
    try {
      ...
      console.log('Live reload not available:', error instanceof Error ? error.message : String(error));
    }
  }
}
