...
const iconsPath = app.isPackaged ? join(resourcesPath, './icons') : join(app.getAppPath(), 'build/icons');
const daemon = app.isPackaged ? join(app.getAppPath(), '../', '/daemon') : join(app.getAppPath(), 'daemon');
const nativeBinding = app.isPackaged
    ? join(resourcesPath, '/bin/better_sqlite3.node')
    : join(app.getAppPath(), '/node_modules/better-sqlite3/build/Release/better_sqlite3.node');
const dbPath = join(appDirectories.mainDir, 'images_database.sqlite3');
const binDirectory = app.isPackaged ? join(resourcesPath, 'bin') : join(app.getAppPath(), '/bin');
