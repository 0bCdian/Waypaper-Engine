...
export class DaemonMonitor {
  ...
  startMonitoring(intervalMs: number = 1000): void {
    if (this.isMonitoring) {
      console.log('DaemonMonitor: Already monitoring');
      return;
    }

    this.isMonitoring = true;
    console.log(`DaemonMonitor: Starting health checks every ${intervalMs}ms`);
  ...
  stopMonitoring(): void {
    ...
    console.log('DaemonMonitor: Stopping health checks');
  ...
  private async performHealthCheck(): Promise<void> {
    try {
      ...
      if (pingResult) {
        ...
        try {
          const daemonStatus = await goDaemonClient.getDaemonStatus();
      ...
      if (statusChanged) {
        console.log('DaemonMonitor: Status changed, broadcasting update');
      ...
      if (statusChanged) {
        console.log('DaemonMonitor: Status changed to error, broadcasting update');
        this.broadcastStatusUpdate();
      }
    }
  }
  ...
  async restartDaemon(): Promise<{ success: boolean; error?: string }> {
    try {
      console.log('DaemonMonitor: Restarting daemon...');
      
      // Stop the daemon first
      try {
        await goDaemonClient.stopDaemon();
        console.log('DaemonMonitor: Daemon stopped successfully');
      } catch (error) {
      ...
      try {
        ...
        console.log('DaemonMonitor: Daemon started successfully');
  ...
  async startDaemon(): Promise<{ success: boolean; error?: string }> {
    try {
      console.log('DaemonMonitor: Starting daemon...');
  ...
  async stopDaemon(): Promise<{ success: boolean; error?: string }> {
    try {
      console.log('DaemonMonitor: Stopping daemon...');
      await goDaemonClient.stopDaemon();
      console.log('DaemonMonitor: Daemon stopped successfully');
  ...
  cleanup(): void {
    this.stopMonitoring();
    this.windows.clear();
    console.log('DaemonMonitor: Cleaned up');
  }
}
