...
test.describe("Gallery", () => {
	test("images list is empty on fresh daemon", async ({ api }) => {
		...
		const body = res.data as { data: unknown[]; pagination: { total_items: number } };
	...
	test("import images and verify they appear", async ({ api, daemon }) => {
		...
		const listData = listRes.data as { data: { id: number; name: string }[]; pagination: { total_items: number } };
	...
	test("get single image by ID", async ({ api, daemon }) => {
		...
		const img = getRes.data as { id: number; name: string; width: number; height: number };
	...
	test("delete images", async ({ api, daemon }) => {
		...
		const listRes = await api.get("/images?page=1&per_page=50");
		const ids = (listRes.data as { data: { id: number }[] }).data.map((img) => img.id);
		...
		const afterData = afterRes.data as { data: unknown[]; pagination: { total_items: number } };
