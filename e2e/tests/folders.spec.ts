...
test.describe("Folders", () => {
	...
	test("create folder", async ({ api }) => {
		...
		const folder = res.data as { id: number; name: string; parent_id: number | null };
	...
	test("get folder path (breadcrumbs)", async ({ api }) => {
		...
		const childRes = await api.post("/folders", { name: "Level2", parent_id: parent.id });
	...
	test("rename folder", async ({ api }) => {
		...
		const updateRes = await api.patch(`/folders/${created.id}`, { name: "New Name" });
