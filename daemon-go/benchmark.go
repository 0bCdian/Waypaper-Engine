package main

import (
	"fmt"
	"testing"
	"time"
)

// Benchmark the old linear search approach
func BenchmarkLinearSearch(b *testing.B) {
	// Create a map with 1M existing files
	existingFiles := make(map[string]bool)
	for i := 1; i <= 1000000; i++ {
		existingFiles[fmt.Sprintf("image(%d).jpg", i)] = true
	}

	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		// Simulate the old linear search approach
		originalFileName := "image.jpg"
		ext := ".jpg"
		fileNameWithoutExt := "image"

		uniqueFileName := originalFileName
		count := 1

		for existingFiles[uniqueFileName] {
			uniqueFileName = fmt.Sprintf("%s(%d)%s", fileNameWithoutExt, count, ext)
			count++
		}
	}
}

// Benchmark the new binary search approach
func BenchmarkBinarySearch(b *testing.B) {
	// Create a map with 1M existing files
	existingFiles := make(map[string]bool)
	for i := 1; i <= 1000000; i++ {
		existingFiles[fmt.Sprintf("image(%d).jpg", i)] = true
	}

	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		// Use the new binary search approach
		originalFileName := "image.jpg"
		ext := ".jpg"
		fileNameWithoutExt := "image"

		if !existingFiles[originalFileName] {
			continue
		}

		// Binary search for the highest existing number
		left, right := 1, 1048576
		for left <= right {
			mid := (left + right) / 2
			testName := fmt.Sprintf("%s(%d)%s", fileNameWithoutExt, mid, ext)

			if existingFiles[testName] {
				left = mid + 1
			} else {
				right = mid - 1
			}
		}

		// The next available number
		_ = fmt.Sprintf("%s(%d)%s", fileNameWithoutExt, right+1, ext)
	}
}

// Test the correctness of both approaches
func TestCorrectness(t *testing.T) {
	// Create a map with some existing files
	existingFiles := make(map[string]bool)
	existingFiles["image.jpg"] = true
	existingFiles["image(1).jpg"] = true
	existingFiles["image(2).jpg"] = true
	existingFiles["image(5).jpg"] = true
	existingFiles["image(10).jpg"] = true

	// Test linear search approach
	originalFileName := "image.jpg"
	ext := ".jpg"
	fileNameWithoutExt := "image"

	uniqueFileName := originalFileName
	count := 1

	for existingFiles[uniqueFileName] {
		uniqueFileName = fmt.Sprintf("%s(%d)%s", fileNameWithoutExt, count, ext)
		count++
	}

	expected := "image(3).jpg" // Should find the first gap
	if uniqueFileName != expected {
		t.Errorf("Linear search: expected %s, got %s", expected, uniqueFileName)
	}

	// Test binary search approach
	if !existingFiles[originalFileName] {
		t.Fatal("Original file should exist")
	}

	left, right := 1, 1048576
	for left <= right {
		mid := (left + right) / 2
		testName := fmt.Sprintf("%s(%d)%s", fileNameWithoutExt, mid, ext)

		if existingFiles[testName] {
			left = mid + 1
		} else {
			right = mid - 1
		}
	}

	binaryResult := fmt.Sprintf("%s(%d)%s", fileNameWithoutExt, right+1, ext)
	if binaryResult != expected {
		t.Errorf("Binary search: expected %s, got %s", expected, binaryResult)
	}
}

func main() {
	// Run a quick performance comparison
	fmt.Println("Performance comparison for finding unique filename with 1M existing files:")

	// Create test data
	existingFiles := make(map[string]bool)
	for i := 1; i <= 1000000; i++ {
		existingFiles[fmt.Sprintf("image(%d).jpg", i)] = true
	}

	// Test linear search
	start := time.Now()
	for i := 0; i < 100; i++ {
		originalFileName := "image.jpg"
		ext := ".jpg"
		fileNameWithoutExt := "image"

		uniqueFileName := originalFileName
		count := 1

		for existingFiles[uniqueFileName] {
			uniqueFileName = fmt.Sprintf("%s(%d)%s", fileNameWithoutExt, count, ext)
			count++
		}
	}
	linearTime := time.Since(start)

	// Test binary search
	start = time.Now()
	for i := 0; i < 100; i++ {
		originalFileName := "image.jpg"
		ext := ".jpg"
		fileNameWithoutExt := "image"

		if !existingFiles[originalFileName] {
			continue
		}

		left, right := 1, 1048576
		for left <= right {
			mid := (left + right) / 2
			testName := fmt.Sprintf("%s(%d)%s", fileNameWithoutExt, mid, ext)

			if existingFiles[testName] {
				left = mid + 1
			} else {
				right = mid - 1
			}
		}

		_ = fmt.Sprintf("%s(%d)%s", fileNameWithoutExt, right+1, ext)
	}
	binaryTime := time.Since(start)

	fmt.Printf("Linear search: %v for 100 iterations\n", linearTime)
	fmt.Printf("Binary search: %v for 100 iterations\n", binaryTime)
	fmt.Printf("Speedup: %.2fx\n", float64(linearTime)/float64(binaryTime))
}
