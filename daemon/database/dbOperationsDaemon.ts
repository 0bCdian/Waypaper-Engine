...
function createConnection() {
    try {
        const db = Database(join(homedir(), '.waypaper_engine', 'images_database.sqlite3'), {
            fileMustExist: true,
            timeout: 10000
        });
        return db;
...
const dbOperations = {
    ...
    getImagesInPlaylist: function (playlistID: number) {
        try {
            ...
            for (let current = 0; current < imagesInPlaylist.length; current++) {
                const currentName = dbOperations.getImageNameFromID(imagesInPlaylist[current].imageID);
                if (currentName !== null) {
                    imagesArray.push({
                        name: currentName,
    ...
    getImageNameFromID: function (imageID: number) {
        try {
            const selectImage = dbOperations.connection.prepare(`SELECT name FROM ${dbTables.Images} WHERE id = ?`);
    ...
    updatePlaylistCurrentIndex: function (imageIndex: number, playlistName: string) {
        try {
    ...
    readAppConfig: function () {
        try {
            const resultDB = dbOperations.connection.prepare(`SELECT * FROM ${dbTables.appConfig}`).all();
            ...
            const parsedConfig: initialAppConfig = {
                ...
                playlistStartOnFirstImage: Boolean(appConfigDB.playlistStartOnFirstImage),
    ...
    readSwwwConfig: function () {
        try {
            const [swwwConfigObject] = dbOperations.connection.prepare(`SELECT * FROM ${dbTables.swwwConfig}`).all();
    ...
    readCurrentPlaylistID: function () {
        const currentPlaylistID = dbOperations.connection.prepare(`SELECT * FROM ${dbTables.activePlaylist}`).all() as
            | [{ playlistID: number }]
            | [];
    ...
    readAllImagesInDB() {
        const selectImages = dbOperations.connection.prepare(`SELECT * FROM ${dbTables.Images}`);
        try {
    ...
    setActivePlaylistToNull() {
        try {
            dbOperations.connection.prepare(`UPDATE ${dbTables.activePlaylist} SET playlistID=0`).run();
        } catch (error) {
            throw new Error(`Could not connect to the database\n Error:\n${error as string}`);
        }
    }
