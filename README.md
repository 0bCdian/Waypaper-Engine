...
**wayland-utauri / HTML wallpapers**: by default, HTML wallpapers cannot use `fetch`/XHR to the network (WebKit `connect-src 'none'`). Enable **`backend.wayland-utauri.allow_network_wallpapers`** in config (Settings → Backend when using wayland-utauri) or set it in TOML; the daemon syncs the policy to a running `wayland-utauri` and passes the spawn flag when it starts the child. Wallpaper JSON cannot grant network access. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#14-security-daemon-socket--html-wallpapers).
